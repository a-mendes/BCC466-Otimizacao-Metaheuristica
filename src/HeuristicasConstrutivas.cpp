...
Solucao VizinhoMaisProximo(Instancia &inst)
{
    ...
    while (!C.empty())
    {
        int id = melhorCandidatoVMP(inst, C, sol.at(sol.size() - 1));
        sol.push_back(id);
        C.erase(id);
    }
...
Solucao VMP_ParcialmenteGuloso(Instancia &inst)
{
    ...
    while (!C.empty())
    {
        ...
        int n_melhores = 3;
        if (C.size() >= n_melhores)
        {
            vector<int> melhores_C;
            for (int i = 0; i < n_melhores; i++)
            {
                int id = melhorCandidatoVMP(inst, C_aux, sol.at(sol.size() - 1));
                melhores_C.push_back(id);
                C_aux.erase(id);
            }

            int melhor_aleatorio = rand() % n_melhores;
            int melhor_id = melhores_C.at(melhor_aleatorio);
            C.erase(melhor_id);

            cout << "melhor id " << melhor_id << endl;
            sol.push_back(melhor_id);
        }
